...
async def start_anthropic_upstream(port, handler):
    app = web.Application()
...
@pytest.mark.asyncio
async def test_concurrent_anthropic_requests_keep_vaults_isolated(
    unused_tcp_port,
):
    upstream_payloads = []
    ...
    upstream_runner = await start_anthropic_upstream(
        unused_tcp_port, upstream_handler
    )
    events = []
    proxy = PrivacyProxyServer(
    ...
    async def send(session: ClientSession, phone: str) -> str:
        response = await session.post(
            ...
            headers={
                "X-Api-Key": "sk-ant-transportkey123456",
    ...
    try:
        ...
        assert_secrets_absent(
            events, ["sk-ant-transportkey123456"]
        )
    finally:
        await proxy.stop()
        await upstream_runner.cleanup()
